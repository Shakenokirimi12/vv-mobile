///
/// VoicevoxModelInfo.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.voicevox

import androidx.annotation.Keep
import com.facebook.proguard.annotations.DoNotStrip
import java.util.Objects


/**
 * Represents the JavaScript object/struct "VoicevoxModelInfo".
 */
@DoNotStrip
@Keep
data class VoicevoxModelInfo(
  @DoNotStrip
  @Keep
  val id: String,
  @DoNotStrip
  @Keep
  val filename: String,
  @DoNotStrip
  @Keep
  val sizeBytes: Double,
  @DoNotStrip
  @Keep
  val downloadURL: String,
  @DoNotStrip
  @Keep
  val vvmId: String,
  @DoNotStrip
  @Keep
  val characters: Array<VoicevoxCharacter>,
  @DoNotStrip
  @Keep
  val isDownloaded: Boolean
) {
  /* primary constructor */

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (other !is VoicevoxModelInfo) return false
    return Objects.deepEquals(this.id, other.id)
      && Objects.deepEquals(this.filename, other.filename)
      && Objects.deepEquals(this.sizeBytes, other.sizeBytes)
      && Objects.deepEquals(this.downloadURL, other.downloadURL)
      && Objects.deepEquals(this.vvmId, other.vvmId)
      && Objects.deepEquals(this.characters, other.characters)
      && Objects.deepEquals(this.isDownloaded, other.isDownloaded)
  }

  override fun hashCode(): Int {
    return arrayOf<Any?>(
      id,
      filename,
      sizeBytes,
      downloadURL,
      vvmId,
      characters,
      isDownloaded
    ).contentDeepHashCode()
  }

  companion object {
    /**
     * Constructor called from C++
     */
    @DoNotStrip
    @Keep
    @Suppress("unused")
    @JvmStatic
    private fun fromCpp(id: String, filename: String, sizeBytes: Double, downloadURL: String, vvmId: String, characters: Array<VoicevoxCharacter>, isDownloaded: Boolean): VoicevoxModelInfo {
      return VoicevoxModelInfo(id, filename, sizeBytes, downloadURL, vvmId, characters, isDownloaded)
    }
  }
}
